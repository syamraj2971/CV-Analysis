...
def starttest(cid,tid,uid):
	...
	if "submit" in request.form:
		...
		qry="insert into answer values(null,'%s','%s','%s','%s',curdate())"%(tid,session['id'],a,counter)
		session['ansid']=insert(qry)
		x="select * from application where user_id='%s' and company_id='%s'"%(uid,cid)
		xr=select(x)
		# if xr:
		# 	u="update application set mark=mark+'%s' where user_id='%s' and company_id='%s'"%(uid,cid)
		# 	update(u)
		# return redirect(url_for('refresh_page'))
		
		return redirect(url_for('app.viewscore'))
